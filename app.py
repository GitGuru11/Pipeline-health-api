...
def get_table_data():
    ...
    try:
        query = f"SELECT * FROM 'overview_copy' WHERE parent = '{parent_node}'"
        result = clickhouse_handler.execute_query(query)
        fields = (
            'node',
...
def get_chart_data():
    ...
    try:
        query = f"SELECT * FROM 'overview_copy' WHERE node = '{node}'"
        result = clickhouse_handler.execute_query(query)
        fields = (
            'node',
            'coverage_4rq', 'coverage_threshold', 'qualification_4rq', 'qualification_threshold', 'maturity_4rq', 'maturity_threshold',
...
def get_second_chart_data():
    ...
    try:
        query = f"SELECT * FROM 'health_kpi_full' WHERE node = '{node}'"
        query2 = f"SELECT * FROM 'quarterly_figures' WHERE node = '{node}'"
        result = clickhouse_handler.execute_query(query)
        result2 = clickhouse_handler.execute_query(query2)
        fields = (
